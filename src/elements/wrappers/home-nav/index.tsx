...
export const HomeNav = () => {
    ...
    return (<nav className={'grid grid-cols-12 items-center bg-slate-800 text-slate-50'}>
        ...
        <ul className={'flex p-2 col-start-4 col-span-full'}>
            { options.map(option => <li><Link to={option.path} className={'hover:underline'}>{ option.label }</Link></li>)}
        </ul>
    </nav>)
}
